      ...
      <div className="mx-auto max-w-6xl px-5 py-16 sm:px-8">
        <div className="grid gap-12 md:grid-cols-[1.3fr_1fr_1fr_1fr]">
          <div>
            ...
            <p className="mt-3 max-w-xs text-sm text-parchment/50">
            ...
            <div className="mt-5 flex gap-4">
              {SOCIALS.map(({ icon: Icon, href, label }) => (
                <a
                  ...
                  href={href}
                  aria-label={label}
                  className="text-parchment/50 transition-colors hover:text-brass"
                >
          ...
          {COLUMNS.map((col) => (
            <div key={col.heading}>
              <h3 className="font-sans text-[0.72rem] uppercase tracking-[0.18em] text-parchment">
                {col.heading}
              </h3>
              <ul className="mt-4 space-y-2.5">
                {col.links.map((link) => (
                  <li key={link.label}>
                    <Link
                      href={link.href}
                      className="text-sm text-parchment/50 transition-colors hover:text-brass"
                    >
                      {link.label}
                    </Link>
                  </li>
        ...
        <div className="mt-14 border-t border-parchment/10 pt-6 text-center text-xs text-parchment/50/70">
